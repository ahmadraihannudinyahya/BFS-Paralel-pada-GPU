#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_NODES 5000000
#define MAX_EDGES 50000000 

struct CSR {
    int numVertices;
    int numEdges;
    int *row_ptr;
    int *col_idx;
};

void save_csr_binary(const char* filename, struct CSR* graph) {
    FILE* f = fopen(filename, "wb");
    if (!f) {
        perror("Gagal membuat file bin");
        exit(1);
    }

    fwrite(&graph->numVertices, sizeof(int), 1, f);
    fwrite(&graph->numEdges, sizeof(int), 1, f);
    fwrite(graph->row_ptr, sizeof(int), graph->numVertices + 1, f);
    fwrite(graph->col_idx, sizeof(int), graph->numEdges, f);

    fclose(f);
}

int main(int argc, char* argv[]) {

    if (argc < 2) {
        printf("Penggunaan: %s <nama_file>\n", argv[0]);
        return 1;
    }

    char filepath[256];
    snprintf(filepath, sizeof(filepath), "datasets/subgraph/%s", argv[1]);

    FILE* file = fopen(filepath, "r");
    if (!file) {
        perror("Gagal membuka file");
        return 1;
    }

    int *degree = (int*)calloc(MAX_NODES, sizeof(int));

    int src, dest;
    int maxNode = 0;
    int edgeCount = 0;

    char line[256];


    while (fgets(line, sizeof(line), file)) {
        if (line[0] == '#' || line[0] == '\n') continue;

        if (sscanf(line, "%d %d", &src, &dest) == 2) {

            if (src >= MAX_NODES || dest >= MAX_NODES) {
                printf("ERROR: node melebihi MAX_NODES (%d, %d)\n", src, dest);
                exit(1);
            }

            degree[src]++;
            edgeCount++;

            if (src > maxNode) maxNode = src;
            if (dest > maxNode) maxNode = dest;
        }
    }

    int numVertices = maxNode + 1;

    struct CSR graph;
    graph.numVertices = numVertices;
    graph.numEdges = edgeCount;

    graph.row_ptr = (int*)malloc((numVertices + 1) * sizeof(int));
    graph.col_idx = (int*)malloc(edgeCount * sizeof(int));


    graph.row_ptr[0] = 0;
    for (int i = 1; i <= numVertices; i++) {
        graph.row_ptr[i] = graph.row_ptr[i - 1] + degree[i - 1];
    }

    int *temp = (int*)malloc(numVertices * sizeof(int));
    memcpy(temp, graph.row_ptr, numVertices * sizeof(int));

    rewind(file);

    while (fgets(line, sizeof(line), file)) {
        if (line[0] == '#' || line[0] == '\n') continue;

        if (sscanf(line, "%d %d", &src, &dest) == 2) {

            int pos = temp[src]++;

            if (pos >= edgeCount) {
                printf("ERROR: overflow CSR pos=%d\n", pos);
                exit(1);
            }

            graph.col_idx[pos] = dest;
        }
    }

    fclose(file);
    free(temp);
    free(degree);

    printf("Node: %d, Edge: %d\n", graph.numVertices, graph.numEdges);

    char outpath[256];
    snprintf(outpath, sizeof(outpath), "bin/datasets/csr/%s.bin", argv[1]);

    save_csr_binary(outpath, &graph);

    printf("✔ CSR berhasil disimpan ke: %s\n", outpath);

    free(graph.row_ptr);
    free(graph.col_idx);

    return 0;
}